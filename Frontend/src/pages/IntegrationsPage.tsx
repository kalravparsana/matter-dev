...
import {
  ...
  integrationMeta,
  platformCatalog,
  inputTriggers as mockInputTriggers,
  outputAgents as mockOutputAgents,
} from '@/data/mattar';
...
import { fetchIntegrations, startSlackOAuth, startGmailOAuth, connectGranola } from '@/lib/api/integrations';
...
export default function IntegrationsPage() {
  ...
  const { data: integrations, refresh } = useApiData(
    mockIntegrations,
    fetchIntegrations,
  );
          ...
          {filtered.map((item) =>
            item.connection ? (
              <ConnectedPlatformCard
                ...
                inputCount={mockInputTriggers.filter((t) => t.integration === item.type).length}
                outputCount={mockOutputAgents.filter((a) => a.integration === item.type).length}
              />
            ) : (
      ...
      {showConnect && (
        <ConnectModal
          ...
          onConnect={async (type) => {
            if (!isApiConfigured()) {
              setConnectError('API is not configured. Set VITE_API_BASE_URL to connect platforms.');
              return;
            }
            setConnecting(true);
            ...
            try {
              if (type === 'slack') {
                ...
                const key = window.prompt('Enter your Granola API key');
                if (!key) return;
                await connectGranola(key);
                await refresh();
...
function ConnectModal({
  ...
  onConnect: (type: CoreIntegrationType) => void;
  connecting: boolean;
  ...
  return (
    ...
    >
      ...
      >
            ...
            return (
              <li key={platform.type}>
                <button
                  ...
                  onClick={() => !isConnected && onConnect(platform.type)}
        ...
        {connectError && (
          <p className="mt-3 font-sans text-xs text-destructive" role="alert">
            {connectError}
          </p>
        )}
        <button
