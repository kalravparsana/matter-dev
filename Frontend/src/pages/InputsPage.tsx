...
import { fetchInputTriggers, updateInputTrigger } from '@/lib/api/data';
...
export default function InputsPage() {
  ...
  const { data: apiTriggers, refresh } = useApiData(mockInputTriggers, fetchInputTriggers);
  const [localTriggers, setLocalTriggers] = useState<InputTrigger[] | null>(null);
  const triggers = localTriggers ?? apiTriggers;
  ...
  const toggleTrigger = useCallback(async (id: string) => {
    ...
    setLocalTriggers(
      triggers.map((t) => (t.id === id ? { ...t, enabled: nextEnabled } : t)),
    );
    if (isApiConfigured()) {
      try {
        await updateInputTrigger(id, nextEnabled);
        await refresh();
        setLocalTriggers(null);
      } catch {
        setLocalTriggers(null);
      }
    }
