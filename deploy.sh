...
patch_dotenv_key() {
  ...
  local key="$2"
  local value="$3"
  node "$ROOT/scripts/patch-dotenv.mjs" "$key" "$value" "$env_file"
}
...
deploy_cloudformation_layer() {
  ...
  (
    ...
    if aws cloudformation describe-stacks --stack-name "$stack_name" >/dev/null 2>&1; then
      ...
      update_status="$(aws cloudformation describe-stacks --stack-name "$stack_name" \
        --query 'Stacks[0].StackStatus' --output text 2>/dev/null || echo "")"
      if [[ "$update_status" == *"_IN_PROGRESS" ]]; then
