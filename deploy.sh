...
# 1. Capture backend stack outputs: ApiBaseUrl, CognitoUserPoolId, CognitoClientId, CognitoDomain
# 2. Write Frontend/.env with VITE_API_BASE_URL and Cognito vars
# 3. cd Frontend && npm ci && npm run build
# 4. aws s3 sync Frontend/dist s3://<S3BucketName> --delete
# 5. aws cloudfront create-invalidation --distribution-id <DistributionId> --paths "/*"
# 6. cd backend && npm run seed (with SEED_USER_ID from Cognito sub)
